...
def to_tensor_and_norm(imgs, labels):
    ...
    # 为了兼容单极化 SAR，归一化不再固定写死为 3 通道，而是按输入实际通道数生成 mean/std
    imgs = [TF.normalize(img, mean=[0.5] * img.shape[0], std=[0.5] * img.shape[0])
            for img in imgs]
    return imgs, labels
...
class CDDataAugmentation:
    ...
    def __init__(
        ...
        # 对 SAR 数据通常不做颜色扰动，因此将该开关暴露给上层控制
    ...
    def transform(self, imgs, labels, to_tensor=True):
        ...
        if to_tensor:
            ...
            # 同样按实际通道数归一化，保证单通道和三通道数据都能直接复用该流程
            imgs = [TF.normalize(img, mean=[0.5] * img.shape[0], std=[0.5] * img.shape[0])
                    for img in imgs]
