...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        # 每次训练运行单独创建一个日志文件，避免不同实验相互覆盖
        self.logs_dir = os.path.join(args.checkpoint_dir, 'Logs')
        os.makedirs(self.logs_dir, exist_ok=True)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        logger_path = os.path.join(self.logs_dir, f'train_{timestamp}.txt')
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            self.logger.write('正在加载上一次训练的检查点...\n')
            ...
            self.logger.write('起始轮次 = %d, 历史最佳精度 = %.4f (出现在第 %d 轮)\n' %
                  (self.epoch_to_start, self.best_val_acc, self.best_epoch_id))
            self.logger.write('\n')

        elif self.args.pretrain is not None:
            print("正在从以下路径初始化预训练权重: " + self.args.pretrain)
    ...
    def _collect_running_batch_states(self):
        ...
        if np.mod(self.batch_id, 100) == 1:
            message = '训练阶段: %s. [%d,%d][%d,%d], 每秒样本数: %.2f, 预计剩余: %.2f小时, 损失: %.5f, 当前mf1: %.5f\n' %\
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            self.logger.write('本轮训练耗时: %s\n\n' % self._format_duration(train_epoch_duration))
        ...
        self.logger.write('本次训练总耗时: %s\n' % self._format_duration(total_train_duration))
