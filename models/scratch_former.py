...
class SARMoEBlock(nn.Module):
    """
    面向 SAR 的轻量级 MoE 模块。
    通过不同感受野的深度可分离卷积专家，对斑点噪声、局部纹理和上下文信息进行自适应加权。
    """

    def __init__(self, channels):
        super().__init__()
        hidden_channels = max(channels // 4, 4)
    ...
    def forward(self, x):
        ...
        mixed = (expert_outputs * gate.unsqueeze(2)).sum(dim=1)
...
class DecoderTransformer(nn.Module):
    ...
    def __init__(self, align_corners=True, in_channels=[64, 128, 320, 512], embedding_dim=256,
                 output_nc=2, decoder_softmax=False, use_moe=False):
        ...
        if self.use_moe:
            # 在双时相差异特征和最终融合特征上加入 MoE，有利于 SAR 场景下的多尺度自适应建模
            self.scale_moe = nn.ModuleList([SARMoEBlock(self.embedding_dim) for _ in range(5)])
...
class ScratchFormer(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256, use_moe=False):
        ...
        # 对单通道 SAR，在进入共享编码器前先做一次轻量自适应增强
        self.input_adapter = SARMoEBlock(input_nc) if self.use_moe else nn.Identity()
